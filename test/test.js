...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( nanmin.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the minimum value of a one-dimensional ndarray', function test( t ) {
	...
	v = nanmin( [ vector( x, 7, 1, 0 ) ] );
	t.strictEqual( v, -4.0, 'returns expected value' );

	x = [ -4.0, NaN, -5.0 ];
	v = nanmin( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( v, -5.0, 'returns expected value' );

	x = [-0.0, 0.0, NaN, -0.0 ];
	v = nanmin( [ vector( x, 4, 1, 0 ) ] );
	t.strictEqual( isNegativeZero( v ), true, 'returns expected value' );

	x = [ NaN ];
	v = nanmin( [ vector( x, 1, 1, 0 ) ] );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	x = [ NaN, NaN ];
	v = nanmin( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
